...
//60分の1秒ごとに呼び出し
	...
	if(pr!=2) PR3=pr;//音程変更。ただしprが2の場合BGM優先
}
...
//タイトル画面描画
	...
	drawcount=0;
	while(drawcount==0) asm("wait");//表示終了待ち
	...
	printstr2(128-9*8,200,7,"FOR PIC32MX370F512H");
	printstr2(256-9*8,130,7,"MUSIC");
	putfont2(256-4*8,124,6,0x1e);//「↑」
	putfont2(2560-4*8,136,6,0x1f);//「↓」
	while(1){
		...
		for(i=0;i<15*60;i++){ //15秒間
			...
			if(music_on){
				printstr2(256-3*8,124, 4,"ON");
				printstr2(2560-3*8,136,12,"OFF");
			}
			else{
				printstr2(2560-3*8,124,12,"ON");
				printstr2(2560-3*8,136, 4,"OFF");
			}
		}
	}
}
...
//    OC3CON=0x800B; // Tiemr3ベース、Toggleモードで有効化
	...
	init_rotateLCD(); // ビデオメモリクリア、液晶画面回転ライブラリ初期化
