...
void putbmpmn_fixarea(unsigned char x,unsigned char y,char m,char n,const unsigned char bmp[]){
//画面上部固定領域にビットマップ表示
...
void clrbmpmn_fixarea(unsigned char x,unsigned char y,char m,char n){
//画面上部固定領域の表示消去
//座標(x,y)からm*nの領域を消去
	unsigned char i;
	unsigned char *p;
...
void putbmpmn2(unsigned char x,unsigned char y,char m,char n,unsigned char bmp[]){
...
void putbmpmn3(unsigned char x,unsigned char y,char m,char n,const unsigned char bmp[]){
// 横m*縦nの画像配列bmp[]をVRAM座標(x,y)に表示
// 画面上下、左右は環状につながっているものとする
